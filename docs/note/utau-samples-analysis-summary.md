...
UTAU 分析の後続作業として、生成 WAV とリファレンス WAV を比較する波形比較ツールを作成した。主なスクリプトは `research/scripts/audio_utils.py`、`research/scripts/compare_waveforms.py`、`research/scripts/compare_waveform_pairs.py`、`research/scripts/iterate_vowel_match.py` である。

比較ツールでは、波形 RMSE、正規化相互相関、対数スペクトル距離、スペクトル収束、F0 差、フォルマント MAE、DTW スペクトル距離、RMS 立ち上がり差などを CSV/JSON/PNG として出力できる。

このツールを使い、`maoto/単独音/あ.wav` を対象に `/a/` の 10 回反復生成実験を行った。改善版では複合スコアが `88.669039` から `48.588397` へ改善し、`formant_mae_hz` は `396.535406` から `44.510082` へ下がった。

Web プロトタイプには、この結果を `tuned` プリセットとして反映した。現時点では `/a/` の F1/F3 とゲインのみを調整値として採用し、`/i/ /u/ /e/ /o/` は UTAU 値を初期値として残している。改善版生成で使った `spectral_match` は上限確認用の補正であり、そのまま Web へ移植せず、source tilt、帯域別ゲイン、フォルマントゲインへ分解して扱う。
